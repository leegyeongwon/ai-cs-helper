...
def test_litellm_base_url_prefers_local_override(monkeypatch):
    ...
    assert llm_demo.litellm_base_url() == "http://localhost:9000/v1"
...
def test_litellm_base_url_ignores_docker_internal_host(monkeypatch):
    ...
    assert llm_demo.litellm_base_url() == "http://localhost:4000/v1"
...
def test_litellm_base_url_uses_external_base_url(monkeypatch):
    ...
    assert llm_demo.litellm_base_url() == "https://litellm.example.com/v1"
...
def test_ask_returns_message_content(monkeypatch):
    ...
    monkeypatch.setattr(llm_demo, "post_json", fake_post_json)

    assert llm_demo.ask("안녕") == "안녕하세요!"
