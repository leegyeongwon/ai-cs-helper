...
설치 필요 패키지
    uv add ragas openai python-dotenv pandas
    (langchain-openai는 더 이상 필요 없음 — ragas의 llm_factory/embedding_factory가
     openai 패키지를 직접 사용하는 새 방식으로 바뀌면서 langchain 계열 의존성이 빠졌다.)
...
# ---------------------------------------------------------------------------
def build_ragas_llm():
    """LiteLLM 프록시를 RAGAS 평가용 LLM으로 재사용한다 (OpenAI 호환 엔드포인트)."""
    client = OpenAI(
        base_url=litellm_base_url(),
        api_key=env("LITELLM_MASTER_KEY"),
    )
    return llm_factory(env("LITELLM_CHAT_MODEL"), client=client)


def build_ragas_embeddings():
    """
    Upstage 임베딩 API는 OpenAI 호환 스펙(response["data"][0]["embedding"])이라
    OpenAI 클라이언트에 base_url/api_key만 Upstage 것으로 바꿔서 그대로 사용한다.
    """
    client = OpenAI(
